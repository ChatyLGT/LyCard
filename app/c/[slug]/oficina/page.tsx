...
// (project/company/personal), se vuelve con el link de arriba. La mitad de
// abajo ("Vista interna") solo la ve isHost — mismo criterio que ya usa
// /c/[slug] para todo lo demás, no un modelo nuevo.
//
...
export default async function OficinaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [adminScope, memberId, originMemento] = await Promise.all([
    currentAdminScope(),
    currentMemberId(),
    card.kind === "project" && card.memberId ? prisma.originMemento.findUnique({ where: { memberId: card.memberId } }) : Promise.resolve(null),
  ]);
  ...
  const accent = program?.primaryColor || "#C8A15A";
  ...
  const roleLabel = card.puesto?.denominacion || card.title;
  const dashboardHref = card.kind === "project" ? "/m/dashboard/project" : card.kind === "company" ? "/m/dashboard/company" : "/m/dashboard/personal";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0B0A", color: "#F3F0E9", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      ...
      >
        <Link href={`/c/${card.slug}`} style={{ color: accent, font: "600 10px 'Plus Jakarta Sans',sans-serif", letterSpacing: ".1em", textTransform: "uppercase" }}>
          ← Volver a la tarjeta
        </Link>
        <span style={kickerStyle}>Vista pública</span>
      </div>
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "24px 20px 40px", display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {/* Descargas — cáscara honesta, sin archivo real todavía */}
        ...
        {/* KPIs — Portafolio y Malla son reales, Cartera sigue como cáscara (Fase 11-C) */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, textAlign: "center" }}>
            <span style={{ font: "700 17px 'Playfair Display',serif" }}>{officeItems.length}</span>
            <span style={{ ...kickerStyle, fontSize: 7 }}>Portafolio</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, textAlign: "center" }}>
            <span style={{ font: "700 17px 'Playfair Display',serif", color: accent }}>{badge}</span>
        ...
        {/* Origin story (project) / Portafolio (company, personal) — el
            mismo contenido real que antes vivía en el modal "office" */}
        {card.kind === "project" ? (
          origin ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "10px 6px" }}>
            {["Video a redes", "Presupuesto del mes", "WhatsApp 24/7", "Buscar licitaciones", "Flyer promocional", "Recordar pagos"].map((c) => (
              <div key={c} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center" }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(200,161,90,.08)", border: "1px solid rgba(200,161,90,.2)" }} />
        ...
        {/* Simulador — CTA real al OnboardingChat existente, no un dead mock */}
        ...
        {isHost && (
          <>
            <div style={{ margin: "10px 0", padding: "20px 0", borderTop: "1px solid rgba(200,161,90,.35)", borderBottom: "1px solid rgba(200,161,90,.35)", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, textAlign: "center" }}>
              <span style={{ font: "700 9px 'Plus Jakarta Sans',sans-serif", color: "#E5C378", letterSpacing: ".1em", textTransform: "uppercase" }}>Acá termina lo que ve cualquiera</span>
              <span style={{ font: "400 10px 'Plus Jakarta Sans',sans-serif", color: "#8f8578", maxWidth: 260, lineHeight: 1.5 }}>
                Todo lo de abajo solo lo ve {card.name}, logueado, como dueño de esta Oficina.
              </span>
            </div>

            <span style={kickerStyle}>Vista interna · dueño</span>
            <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
              <div style={{ width: 36, height: 36, borderRadius: 999, background: `linear-gradient(160deg,#E5C378,${accent})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ font: "700 13px 'Playfair Display',serif", color: "#0D0D0D" }}>{card.name.charAt(0)}</span>
              </div>
              <div>
                <div style={{ font: "600 13px 'Plus Jakarta Sans',sans-serif" }}>Hola, {card.name}</div>
                <span style={{ font: "400 10px 'Plus Jakarta Sans',sans-serif", color: "#756c5e" }}>{program?.name || "Legacy"} · {badge}</span>
              </div>
            </div>
            ...
            {/* Agenda / equipo — ilustrativo, sin cifra de plata */}
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {/* Accesos directos — los que existen de verdad, enlazan; el
                resto queda atenuado, sin fingir que ya está */}
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <span style={kickerStyle}>Tu Oficina, herramienta de trabajo</span>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "10px 6px" }}>
                <Link href={dashboardHref} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center" }}>
                  <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(200,161,90,.08)", border: "1px solid rgba(200,161,90,.2)" }} />
                  <span style={{ font: "400 9px 'Plus Jakarta Sans',sans-serif", color: "#C2BEB5" }}>Editar Oficina</span>
                </Link>
                {card.kind === "company" ? (
                  <Link href="/m/dashboard/company/network" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center" }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(200,161,90,.08)", border: "1px solid rgba(200,161,90,.2)" }} />
                    <span style={{ font: "400 9px 'Plus Jakarta Sans',sans-serif", color: "#C2BEB5" }}>Mi Red (Malla)</span>
                  </Link>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center", opacity: 0.5 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(200,161,90,.08)", border: "1px solid rgba(200,161,90,.2)" }} />
                ...
                {["Agenda", "Mensajes", "Cartera NashMesh", "Configuración"].map((label) => (
                  <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center", opacity: 0.5 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(200,161,90,.08)", border: "1px solid rgba(200,161,90,.2)" }} />
